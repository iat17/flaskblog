...
class TestArticleViews:
    ...
    def test_create_article(self, test_client, db):
        ...
        response: Response = test_client.post('/articles/', headers={"Authorization": f"Basic {credentials}"}, json={
            'title': 'test1',
            'description': 'test1',
            'body': 'test1'
        })
    ...
    def test_update_article(self, test_client, article_fixt):
        ...
        response: Response = test_client.put(
            f'/articles/{article_fixt.id}/', headers={"Authorization": f"Basic {credentials}"}, json={
                'title': 'test1',
                'description': 'test1',
                'body': 'test1'
            })
    ...
    def test_delete_article(self, test_client, article_fixt):
        ...
        response: Response = test_client.delete(
            f'/articles/{article_fixt.id}/', headers={"Authorization": f"Basic {credentials}"})

        assert response.status_code == 200
        assert article_fixt.is_deleted == True
